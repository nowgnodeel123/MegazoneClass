 ...
 *    B: C# 으로 개발됨.
 *    
 *    A <-> B, 업무 처리 시 서로 연결이 되어야 함.
 *    통신 규약 (네트워크 통신) => main() 에서 전문 반환.
 *    
 *    같은 서버에 A, B 가 존재하는 경우. => main() 반환형을 정수 or 문자.
 *    => batch system. (spring batch)
 *    
 *    
 * 연결된 예외 (chained exception)
 *  - 예외 A 가 예외 B 를 발생시켰다면, A 를 B 의 예외 원인이라고 함.
 *  
 *  SpaceException, MemoryException, InstallException
 *  
 *  SpaceException, MemoryException 는 InstallException 의 예외 원인이라고 할 수 있음.
 *  포함 관계, 다형성.
 *  
 *  
 *  
 ...
 *  										  cause 의 type 은 Throwable 임.
 *  	// 현재 예외 인스턴스의 원인 예외를 등록할 수 있는 변수임.
 *  	// cause 의 type 이 Throwable 이므로, 모든 예외는 발생한 예외의 원인 예외를 등록 가능.
 *  
 *  } catch (MemoryException me) {
 *  }
 *  
 *  발생한 예외를 그냥 처리하면 되는데, 왠지 복잡해진 것 같은..
 *   - 하나의 큰 분류의 예외로 묶어서 관리하고 싶은 경우.
 *     큰 분류의 예외로 catch 해서 처리하려고 하는데, 실제로 발생한 예외를 알 수가 없게 됨.
 *     
 *   - 상속 관계로 exception 을 정의하면 casting 이 필요해짐.
 *     파생된 exception 이 많아지게 되면, casting 의 부담이 증가.
 *     
